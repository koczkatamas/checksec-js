...
def HandleMachOFile(file, options, offset=0):
  ...
  if options.set_no_heap_execution:
    flags |= MH_NO_HEAP_EXECUTION
  elif options.clear_no_heap_execution:
    flags &= ~MH_NO_HEAP_EXECUTION

  if options.set_pie:
    flags |= MH_PIE
  elif options.clear_pie:
    flags &= ~MH_PIE
...
def main(me, args):
  ...
  parser.add_option('--set-no-heap-execution', action='store_true',
                    dest='set_no_heap_execution', default=False,
                    help='Set the MH_NO_HEAP_EXECUTION bit')
  parser.add_option('--clear-no-heap-execution', action='store_true',
                    dest='clear_no_heap_execution', default=False,
                    help='Clear the MH_NO_HEAP_EXECUTION bit')
  parser.add_option('--set-pie', action='store_true',
                    dest='set_pie', default=False,
                    help='Set the MH_PIE bit')
  parser.add_option('--clear-pie', action='store_true',
                    dest='clear_pie', default=False,
